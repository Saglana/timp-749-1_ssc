...
int main() {
    ...
    if (b != NULL)
        ...
        k3 = 0;
    printf("%d %d %d\n", k1, k2, k3);
    (void)scanf("%d", &input); //  5
    push_back(&L, input);
    print_invers(&L); //  6
    (void)scanf("%d", &input); //  7
    push_front(&L, input);
    print(&L); // задание 8
    (void)scanf("%d %d", &n, &input); //  9
    b = findNode(&L, n);
    insert_after(&L, b, input);
    print_invers(&L); //  10
    (void)scanf("%d %d", &n, &input); //  11
    b = findNode(&L, n);
    insert_before(&L, b, input);
    print(&L); //  12
    (void)scanf("%d", &input); // 13
    remove_first(&L, input);
    print_invers(&L); // задание 14
    (void)scanf("%d", &input); // 15
    remove_last(&L, input);
    print(&L); //  16
    clean(&L); //  17
    return 0;
